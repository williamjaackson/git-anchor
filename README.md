...
`git anchor init` sweeps the repo to anchor every local branch and attempts reflog-based parent recovery. It's the primary way to populate state, and is safe to rerun as an idempotent refresh.

Individual commands (`get`, `set-parent`) lazy-create anchors on demand without requiring a sweep. Reads (`list`, `parent`, `resolve`, `children`) return whatever's currently in config.
...
Run `git anchor init` in your repo. It anchors every branch and recovers parents via reflog where possible:
...
After that, individual commands (`get`, `set-parent`) maintain state on demand. Rerun `init` to refresh after creating new branches.

## Commands

### `git anchor init`

Sweeps all local branches: anchors any that don't have one, and attempts reflog-based parent recovery for anchored-but-parentless branches. Idempotent, safe to rerun whenever you want to refresh state.
...
Prints the anchor UUID for `branch`, or the current branch if omitted. Lazy-creates an anchor for that one branch if it doesn't have one. Does **not** run the repo-wide sweep — use `init` for that.
...
Prints the anchor UUIDs of every branch whose parent is `branch` (or the current branch if omitted). `--name` prints branch names instead. Output is sorted alphabetically by branch name; empty output if the branch has no children. Pure read — no side effects.
...
`git anchor init`'s sweep recovers parents from reflog with this priority:
...
| Parent renamed before `git anchor init` is run | No — reflog still names the old branch, but it no longer exists. We detect this and skip (no ghost entries written). |
...
- **Why no `post-checkout` hook (yet)?** A hook could capture parents at branch creation and avoid the need to rerun `init`, but hook installation has awkward interactions with tools like husky that commit their own hooks directory. That's tracked for a later release; for now the tool is intentionally local-only state with `init` as the refresh lever.
...
src/
  ...
  commands/
    ...
    init.ts get.ts parent.ts children.ts resolve.ts list.ts setParent.ts remove.ts version.ts help.ts
```
